...
public class MinecraftTests : ChatIntegrationTest
{
    ...
    protected override Task<string> SetupServerAsync(CancellationToken cancellationToken)
        => SetupPaperServerAsync(cancellationToken);

    protected override Task<string> SetupClientAsync(CancellationToken cancellationToken)
        => SetupMinecraftConsoleClientAsync(cancellationToken);
    ...
    private async Task<string> SetupPaperServerAsync(CancellationToken cancellationToken)
    {
        ...
        var versionsJson = await HttpClient.GetStringAsync("https://api.papermc.io/v2/projects/paper", cancellationToken);
        ...
        var buildsJson = await HttpClient.GetStringAsync($"https://api.papermc.io/v2/projects/paper/versions/{latestVersion}", cancellationToken);
        ...
        var buildInfoJson = await HttpClient.GetStringAsync($"https://api.papermc.io/v2/projects/paper/versions/{latestVersion}/builds/{latestBuild}", cancellationToken);
