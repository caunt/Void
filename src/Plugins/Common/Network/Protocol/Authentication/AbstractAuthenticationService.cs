...
namespace Void.Proxy.Plugins.Common.Network.Protocol.Authentication;
...
public abstract class AbstractAuthenticationService(IEventService events, IPlayerService players) : IPluginService
{
    ...
    protected async ValueTask<AuthenticationResult> AuthenticateServerAsync(ILink link, AuthenticationResult authenticationResult, CancellationToken cancellationToken)
    {
        ...
        {
            var packet = await link.ReceivePacketAsync<IClientboundPacket>(cancellationToken);
    ...
    protected abstract ValueTask<AuthenticationResult> HandleServerPacketAsync(ILink link, IClientboundPacket packet, CancellationToken cancellationToken);
}