...
/// <param name="IsRedirected"><see langword="true"/> if the player arrived by being redirected from another server; <see langword="false"/> if this is the player's initial server connection.</param>
public record PlayerJoinedServerEvent(IPlayer Player, IServer Server, ILink Link, bool IsRedirected) : IScopedEventWithResult<bool>
{
    /// <summary>
    /// Gets or sets a value indicating whether the player will be redirected after this event executes.
    /// </summary>
    public bool WillBeRedirected { get => Result; set => Result = value; }
