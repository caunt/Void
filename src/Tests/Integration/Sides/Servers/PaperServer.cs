...
public class PaperServer : IntegrationSideBase
{
    ...
    public static async Task<PaperServer> CreateAsync(string workingDirectory, HttpClient client, string instanceName, string? version = null, int port = 25565, PaperPlugins plugins = PaperPlugins.All, CancellationToken cancellationToken = default)
    {
        ...
        var paperVersion = version ?? versions.RootElement.GetProperty("versions").EnumerateArray().Last().GetString();

        var buildsJson = await client.GetStringAsync($"https://api.papermc.io/v2/projects/paper/versions/{paperVersion}", cancellationToken);
        ...
        var buildInfoJson = await client.GetStringAsync($"https://api.papermc.io/v2/projects/paper/versions/{paperVersion}/builds/{latestBuild}", cancellationToken);
        ...
        var paperUrl = $"https://api.papermc.io/v2/projects/paper/versions/{paperVersion}/builds/{latestBuild}/downloads/{jarName}";
