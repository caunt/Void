...
public class MineflayerClient : IntegrationSideBase
{
    ...
    public static async Task<MineflayerClient> CreateAsync(string workingDirectory, HttpClient client, CancellationToken cancellationToken = default)
    {
        ...
        await File.WriteAllTextAsync(scriptPath, $$"""
            ...
            const [address, version, messagesJson] = process.argv.slice(2);
    ...
    public Task SendTextMessageAsync(string address, ProtocolVersion protocolVersion, string text, CancellationToken cancellationToken = default) =>
        SendMessagesAsync(address, protocolVersion, [text], cancellationToken);

    public async Task SendMessagesAsync(string address, ProtocolVersion protocolVersion, string[] messages, CancellationToken cancellationToken = default)
    {
        StartApplication(_nodePath, hasInput: false, _scriptPath, address, protocolVersion.MostRecentSupportedVersion, JsonSerializer.Serialize(messages));
