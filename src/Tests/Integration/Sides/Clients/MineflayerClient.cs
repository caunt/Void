...
public class MineflayerClient : IntegrationSideBase
{
        ...
        await File.WriteAllTextAsync(scriptPath, $$"""
            ...
            bot.on('spawn', () => {
                ...
                for (const message of messages) {
                    setTimeout(() => bot.chat(message), delay += 1000);
                }
    ...
    {
        return SendCommandsAsync(address, protocolVersion, new[] { text }, cancellationToken);
    }

    public async Task SendCommandsAsync(string address, ProtocolVersion protocolVersion, IReadOnlyList<string> commands, CancellationToken cancellationToken = default)
    {
        var arguments = new List<string> { _scriptPath, address, protocolVersion.MostRecentSupportedVersion };
        arguments.AddRange(commands);
    ...
    private static bool HandleConsole(string line)
    {
        ...
        if (line.StartsWith("KICK:") && !line.Contains("[object Object]"))
            throw new IntegrationTestException(line);
