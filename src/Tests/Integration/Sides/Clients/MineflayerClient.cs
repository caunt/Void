...
public class MineflayerClient : IntegrationSideBase
{
    ...
    public static TheoryData<ProtocolVersion> SupportedVersions { get; } = [.. ProtocolVersion
                .Range(ProtocolVersion.MINECRAFT_1_21_6, ProtocolVersion.MINECRAFT_1_8)];
    ...
    public static async Task<MineflayerClient> CreateAsync(string workingDirectory, HttpClient client, CancellationToken cancellationToken = default)
    {
        ...
        await File.WriteAllTextAsync(scriptPath, $$"""
            ...
            // Physics has a bug that sends a position packet in the configuration phase when that phase is requested by the server from the previous play phase
            ...
            for (const eventName of eventNamesToLog)
            {
              bot.on(eventName, function ()
              {
