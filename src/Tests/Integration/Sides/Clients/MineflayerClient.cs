...
public class MineflayerClient : IntegrationSideBase
{
    ...
    public static async Task<MineflayerClient> CreateAsync(string workingDirectory, HttpClient client, CancellationToken cancellationToken = default)
    {
        ...
        await File.WriteAllTextAsync(scriptPath, $$"""
            ...
            const [address, version, text, waitTimeMs] = process.argv.slice(2);
            const [host, portString] = address.split(':');
            const port = parseInt(portString ?? '25565', 10);
            const timeout = parseInt(waitTimeMs ?? '5000', 10);
    ...
    {
        await SendCommandAsync(address, protocolVersion, text, TimeSpan.FromSeconds(5), cancellationToken);
    }
    ...
    {
        StartApplication(_nodePath, hasInput: false, _scriptPath, address, protocolVersion.MostRecentSupportedVersion, command, waitTime.TotalMilliseconds.ToString());
