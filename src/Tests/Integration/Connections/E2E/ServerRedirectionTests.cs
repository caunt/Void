...
public class ServerRedirectionTests(ServerRedirectionTests.Fixture fixture) : ConnectionUnitBase, IClassFixture<ServerRedirectionTests.Fixture>
{
    ...
    private const string ExpectedTextServer2 = "hello from server2";
    private const string ExpectedTextServer1 = "hello again server1";
    ...
    public async Task MineflayerSwitchesBetweenServers()
    {
        ...
        {
            await fixture.MineflayerClient.SendTextMessagesAsync($"localhost:{ProxyPort}", ProtocolVersion.MINECRAFT_1_20_3, new[]
            {
            ...
            await fixture.PaperServer2.ExpectTextAsync(ExpectedTextServer2, lookupHistory: true, cancellationTokenSource.Token);
            await fixture.PaperServer1.ExpectTextAsync(ExpectedTextServer1, lookupHistory: true, cancellationTokenSource.Token);

            Assert.Contains(fixture.PaperServer2.Logs, line => line.Contains(ExpectedTextServer2));
            Assert.Contains(fixture.PaperServer1.Logs, line => line.Contains(ExpectedTextServer1));
