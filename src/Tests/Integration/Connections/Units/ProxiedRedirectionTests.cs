...
public class ProxiedRedirectionTests(ProxiedRedirectionTests.TwoPaperServersFixture fixture) : ConnectionUnitBase, IClassFixture<ProxiedRedirectionTests.TwoPaperServersFixture>
{
    ...
    public async Task Mineflayer_RedirectsBetweenServers()
    {
        ...
        var server1ReturnMessage = $"server1-return-{Random.Shared.Next()}";
        ...
        await LoggedExecutorAsync(async () =>
        {
            await fixture.MineflayerClient.SendTextMessagesAsync($"localhost:{ProxyPort}", ProtocolVersion.MINECRAFT_1_20_3,
                [server1Message, $"/server {Server2Name}", server2Message, $"/server {Server1Name}", server1ReturnMessage], cancellationTokenSource.Token);
