...
public class ProxiedServerRedirectionTests(ProxiedServerRedirectionTests.MultiServerFixture fixture)
    : ConnectionUnitBase, IClassFixture<ProxiedServerRedirectionTests.MultiServerFixture>
{
    ...
    {
        var server2Message = $"server2 test #{Random.Shared.Next()}";
        var server1Message = $"server1 test #{Random.Shared.Next()}";

        using var cancellationTokenSource = new CancellationTokenSource(Timeout);

        await LoggedExecutorAsync(async () =>
        {
            await fixture.MineflayerClient.ExecuteCommandsAsync(
                $"localhost:{ProxyPort}",
                ProtocolVersion.MINECRAFT_1_20_3,
                ["/server args-server-2", server2Message, "/server args-server-1", server1Message],
