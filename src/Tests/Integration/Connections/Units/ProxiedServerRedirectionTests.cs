...
public class ProxiedServerRedirectionTests(ProxiedServerRedirectionTests.MultipleServersFixture fixture) : ConnectionUnitBase, IClassFixture<ProxiedServerRedirectionTests.MultipleServersFixture>
{
    ...
    {
        var toServer2 = $"server2 test #{Random.Shared.Next()}";
        var backToServer1 = $"server1 test #{Random.Shared.Next()}";

        using var cancellationTokenSource = new CancellationTokenSource(Timeout);

        await LoggedExecutorAsync(async () =>
        {
            await fixture.MineflayerClient.SendTextMessageAsync($"localhost:{ProxyPort}", ProtocolVersion.MINECRAFT_1_20_3,
                new[]
                {
                    $"/server {Server2Name}",
                    toServer2,
            ...
            await fixture.PaperServer1.ExpectTextAsync(backToServer1, lookupHistory: true, cancellationTokenSource.Token);

            Assert.Contains(fixture.PaperServer2.Logs, line => line.Contains(toServer2));
            Assert.Contains(fixture.PaperServer1.Logs, line => line.Contains(backToServer1));
