...
public class ServerRedirectionTests(ServerRedirectionTests.ProxyTwoPaperServersFixture fixture) : ConnectionUnitBase, IClassFixture<ServerRedirectionTests.ProxyTwoPaperServersFixture>
{
    ...
    public async Task MineflayerMovesBetweenServers()
    {
        ...
        {
            await fixture.MineflayerClient.SendCommandsAsync(
                $"localhost:{ProxyPort}",
                ProtocolVersion.MINECRAFT_1_20_3,
                ["/server args-server-2", "/server args-server-1"],
                cancellationTokenSource.Token);

            var server1Joins = fixture.PaperServer1.Logs.Count(line => line.Contains("joined the game"));
            var server2Joins = fixture.PaperServer2.Logs.Count(line => line.Contains("joined the game"));

            Assert.True(server1Joins >= 2, $"Expected at least 2 joins on server1, but got {server1Joins}");
            Assert.True(server2Joins >= 1, $"Expected at least 1 join on server2, but got {server2Joins}");
        }, fixture.MineflayerClient, fixture.VoidProxy, fixture.PaperServer1, fixture.PaperServer2);
    }

    public class ProxyTwoPaperServersFixture : ConnectionFixtureBase, IAsyncLifetime
    {
        public ProxyTwoPaperServersFixture() : base(nameof(ServerRedirectionTests))
        {
        }
