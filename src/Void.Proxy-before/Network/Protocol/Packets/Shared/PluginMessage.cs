...
public partial struct PluginMessage : IMinecraftPacket<IConfigurePlayState>
{
    ...
    public PluginMessage() : this(0, 0)
    {
        throw new NotSupportedException("Specify direction and size limit to plugin message");
    }

    public PluginMessage(Direction direction, int maxDataSize)
    {
        (Direction, MaxDataSize) = (direction, maxDataSize);
    }
    ...
    public async Task<bool> HandleAsync(IConfigurePlayState state)
    {
    ...
    public int MaxSize()
    {
        return 0 + Encoding.UTF8.GetByteCount(Identifier) + 5 + Data.Length + 5;
    }
