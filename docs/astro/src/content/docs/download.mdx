...
<Tabs>
    ...
    <TabItem label="Development Builds" icon="puzzle">
        ...
        ****
        ...
        {builds.runs.filter(run => run.artifacts.length > 0).map((entry, index) => { 
            return (
                <Card title={`Build #${entry.run_number}`} icon={entry.conclusion === "success" ? "seti:github" : "warning"}>
                    <Badge text={`Build Date: ${new Date(entry.updated_at).toLocaleString().replace(',', '')}`} variant="note" />
                    <br/>
                    <Badge text={`Branch: ${entry.head_branch}`} variant="note" />
                    <br/>
                    <Badge text={entry.display_title} variant="tip" />
                    <br/>
                    <Badge text={`Status: ${entry.conclusion}`} variant={entry.conclusion === "success" ? entry.conclusion : "caution"} />
                    <br/>
                    <br/>
                    <Tabs syncKey="_key">
