...
export const releases = (await getCollection('releases')).sort(
    (leftRelease, rightRelease) => new Date(rightRelease.data.published_at).getTime() - new Date(leftRelease.data.published_at).getTime()
);
export const workflows = await getCollection('workflows');
export const builds = workflows.find(entry => entry.data.path.endsWith('runnable-commit.yml'))?.data ?? { runs: [] };
...
export const wrapUnderSummary = (markdown, headerMarker) => {
  const escapedHeaderMarker = headerMarker.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
  const headerPattern = new RegExp(`^${escapedHeaderMarker}\\s+(.*)`)
  const output = []
  const closingDetailsTag = '<' + '/details>'

  let summary = '', contentLines = []
